...
class TrainableLatentDecoder(nn.Module):
    ...
    at latent resolution (H/8, W/8) back to full binary image space (H, W).
...
class DiffusionDiffModel(nn.Module):
    ...
    def forward(
        ...
        # 1. Real image passed through frozen encoder to obtain latent z0
        with torch.no_grad():
        ...
        # 5. Trainable Decoder decodes Z back to binary mask space
        mask_logits = self.decoder(z_high_dim)
