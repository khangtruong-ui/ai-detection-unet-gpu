    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self.start_epoch >= self.epochs:
            ...
            try:
                ...
                self.logger.warning(
                    f"\n⚠️ Training interrupted by user/system ({type(interrupt).__name__})! Saving emergency checkpoint..."
                )
                try:
                    p_paths = self.ckpt_manager.save_periodic(
                        epoch=locals().get("epoch", self.start_epoch),
                        model=self.model,
                        optimizer=self.optimizer,
                        scheduler=self.scheduler,
                        metrics={"step": self.global_step, "interrupted": True},
                        config=self.config.to_dict() if hasattr(self.config, "to_dict") else dict(self.config),
                        step=self.global_step,
                        scaler=self.scaler,
                        history=self.history,
                    ...
                    self.logger.info(
                        f"💾 Interrupted checkpoint safely saved to '{p_paths.get('latest', p_paths.get('periodic'))}' "
                        f"(Step {self.global_step})"
                    )
                except Exception as save_err:
                    self.logger.error(f"Failed to save emergency checkpoint on interrupt: {save_err}")
                raise interrupt
